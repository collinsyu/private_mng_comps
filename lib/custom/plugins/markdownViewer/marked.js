...
;(function () {
  ...
  block.footnotes.normal.paragraph = replace(block.paragraph)('))+)', '|' + block.footnotes.footnote.source + '))+)')();

  block.footnotes.gfm = {
    footnote: block.footnotes.footnote
  };

  block.footnotes.gfm.paragraph = replace(block.gfm.paragraph)('))+)', '|' + block.footnotes.footnote.source + '))+)')();
  ...
  Renderer.prototype.heading = function (text, level, raw) {
    ...
    */
    ...
    var duplicateIndex = this.options.allheadings.map(function (h) {
      return h.text;
    }).indexOf(escapedText);
    var duplicateText;
    if (duplicateIndex === -1) {
    ...
    return '<h' + level + ' id="' + (duplicateText || escapedText) + '">' + '<a class="anchor" href="#' + (duplicateText || escapedText) + '"><span class="octicon octicon-link"></span></a>' + text + '</h' + level + '>\n';
  };
  ...
  Renderer.prototype.listitem = function (text, checked) {
    ...
    return '<li class="task-list-item">' + '<input type="checkbox" class="task-list-item-checkbox"' + (checked ? ' disabled' : false) + (checked ? ' checked' : '') + '> ' + text + '</li>\n';
  };
  ...
  Renderer.prototype.toc = function (items) {
    var html = '<div id="toc" class="toc">' + '<h3>目录</h3>' + '<ul class="toc-tree">';
    html += items;
    html += '</ul></div>';
    return html;
  };
