...
const getIcon = (icon) => {
  if (typeof icon === 'string' && icon.indexOf('http') === 0) {
    return <img src={icon} alt="icon" className={"_yhq_layout1_icon"} />;
  }
  ...
  */
  ...
  getSubMenuOrItem=(item) => {
    if (item.children && item.children.some(child => child.name)) {
      return (
        <MenuItemGroup className={"_yhq_layout1_grouptitle"}
          title={
            item.icon ? (
              <span>
                {getIcon(item.icon)}
                <span className={"_yhq_layout1_navtitle"}>{item.name}</span>
              </span>
            ) : item.name
            }
